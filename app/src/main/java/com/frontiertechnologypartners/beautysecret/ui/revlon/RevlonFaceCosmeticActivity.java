...
public class RevlonFaceCosmeticActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        product1Layout.setOnClickListener(v -> {
            Intent intent = new Intent(RevlonFaceCosmeticActivity.this, ProductsActivity.class);
            intent.putExtra(PRODUCT_TYPE, 29);
            startActivity(intent);
        });

        product2Layout.setOnClickListener(v -> {
            Intent intent = new Intent(RevlonFaceCosmeticActivity.this, ProductsActivity.class);
            intent.putExtra(PRODUCT_TYPE, 30);
            startActivity(intent);
        });

        product3Layout.setOnClickListener(v -> {
            Intent intent = new Intent(RevlonFaceCosmeticActivity.this, ProductsActivity.class);
            intent.putExtra(PRODUCT_TYPE, 31);
            startActivity(intent);
        });

        product4Layout.setOnClickListener(v -> {
            Intent intent = new Intent(RevlonFaceCosmeticActivity.this, ProductsActivity.class);
            intent.putExtra(PRODUCT_TYPE, 32);
            startActivity(intent);
        });
